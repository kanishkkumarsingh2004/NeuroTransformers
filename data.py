...
# -----------------------------
# Load vocabularies from saved file if available
# -----------------------------
VOCAB_PATH = os.path.join(MODEL_DIR, "vocab.json")
CONFIG_PATH = os.path.join(MODEL_DIR, "config.json")

# Initialize vocabularies as None
SRC_VOCAB = None
TGT_VOCAB = None
PAD_IDX = None
SOS_IDX = None
...
# Try to load saved vocab and config
...
def load_dataset():
    """Load dataset and build vocabularies (only called when needed, e.g., for training)"""
    global pairs, SRC_VOCAB, TGT_VOCAB, PAD_IDX, SOS_IDX, EOS_IDX, src_data, tgt_data, src_vocab_size, tgt_vocab_size
    ...
    if len(pairs) == 0:
        raise ValueError("No valid data pairs found. Please ensure your data files contain lines with '|||' separator for source and target text.")
